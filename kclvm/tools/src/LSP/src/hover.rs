...
mod tests {
    ...
    fn schema_doc_hover_test1() {
        let (file, program, prog_scope, _, gs) = compile_test_file("src/test_data/hover_test/hover.k");
    ...
    fn complex_select_hover() {
        let (file, program, prog_scope, _, gs) = compile_test_file("src/test_data/hover_test/fib.k");
        let pos = KCLPos {
            filename: file.clone(),
            line: 14,
            column: Some(22),
